...
{    
    ...
    TRISC =  0x00; 
    ANSELC = 0x00; // ANSELX = 0 deselects analog mode 
    ANSELB = 0x00;
...
{   
    // Set line and position for displaying
    // If line and position are correct
    if ((_pos>=0&&_pos<=15)&&(_line>=0&&_line<=1)){
        // then set the position and the line
         LCDGoto(_pos, _line);
    } else {
        // do nothing
        return;
